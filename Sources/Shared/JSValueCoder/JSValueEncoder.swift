...
public class JSValueEncoder {
    ...
    public func encode<T: Encodable>(_ value: T, into context: JSContext) throws -> JSValue {
        let encoder = Encoder(
            context: context,
            userInfo: userInfo,
            options: options
...
private extension JSValueEncoder {
    struct Encoder {
...
private extension JSValueEncoder.Encoder {
    @dynamicMemberLookup
...
extension JSValueEncoder.Encoder {
    struct KeyedContainer<Key: CodingKey>: KeyedEncodingContainerProtocol {
        ...
        func encodeNil(forKey key: Key) { encode(JSValue(nullIn: self.encoder.context), forKey: key) }
        func encode(_ value: Bool, forKey key: Key) { encode(JSValue(bool: value, in: self.encoder.context), forKey: key) }
        func encode(_ value: String, forKey key: Key) { encode(JSValue(object: value, in: self.encoder.context), forKey: key) }
        func encode(_ value: Double, forKey key: Key) { encode(JSValue(double: value, in: self.encoder.context), forKey: key) }
        func encode(_ value: Int32, forKey key: Key) { encode(JSValue(int32: value, in: self.encoder.context), forKey: key) }
        func encode(_ value: UInt32, forKey key: Key) { encode(JSValue(uInt32: value, in: self.encoder.context), forKey: key) }
        ...
        func encode<T>(_ value: T, forKey key: Key) throws where T: Encodable {
            ...
            case let value:
                let encoder = JSValueEncoder.Encoder(parent: self.encoder, key: key)
                try value.encode(to: encoder)
