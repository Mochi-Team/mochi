...
extension JSValueDecoder.Decoder {
    struct KeyedContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {
        ...
        func decode(_ type: Bool.Type, forKey key: Key) throws -> Bool { try decodeValue(for: key).tryTo(\.isBoolean, JSValue.toBool, codingPath + [key]) }
        func decode(_ type: String.Type, forKey key: Key) throws -> String { try decodeValue(for: key).tryTo(\.isString, JSValue.toString, codingPath + [key]) }
        func decode(_ type: Int32.Type, forKey key: Key) throws -> Int32 { try decodeValue(for: key).tryTo(\.isNumber, JSValue.toInt32, codingPath + [key]) }
        func decode(_ type: UInt32.Type, forKey key: Key) throws -> UInt32 { try decodeValue(for: key).tryTo(\.isNumber, JSValue.toUInt32, codingPath + [key]) }
        func decode(_ type: Double.Type, forKey key: Key) throws -> Double { try decodeValue(for: key).tryTo(\.isNumber, JSValue.toDouble, codingPath + [key]) }
...
private extension JSValue {
    func tryTo<T, R>(_ conditional: KeyPath<JSValue, Bool>, _ callback: (JSValue) -> () -> T, _ codingKeys: [CodingKey] = []) throws -> R {
