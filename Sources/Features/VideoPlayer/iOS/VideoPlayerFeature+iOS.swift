                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    WithViewStore(
                        ...
                        MoreListingRow(
                            ...
                            items: viewStore.groupId?.options ?? [],
                            itemTitle: \.displayName,
                            noneCallback: viewStore.groupId.flatMap { groupId in
                                groupId.allowsEmptySelection ? { viewStore.send(.didTapSubtitle(for: groupId, nil)) } : nil
                            },
                            selectedCallback: { option in
                                if let groupId = viewStore.groupId {
                                    viewStore.send(.didTapSubtitle(for: groupId, option))
                                }
                            }
                        )
                    }
